...
import { projectsApi, storageApi, TrackSummary } from "../../api/client";
import { AOIMap } from "../Map/AOIMap";
import { TrackPicker } from "../SceneSearch/TrackPicker";

type Step = 1 | 2 | 3;

export function NewProjectWizard({ onDone }: { onDone: () => void }) {
      ...
      <div className="steps" style={{ marginBottom: 32 }}>
        {([
          ...
          <Fragment key={n}>
            ...
            {i < arr.length - 1 && (
              <div className={`step-connector ${step > n ? "done" : ""}`} />
            )}
          </Fragment>
        ))}
      </div>
